...
/// Trait for providing a mechanism to open all pieces of a split zip file.
pub trait Opener {
    /// Open a specific split zip. `index` is guaranteed to be less than
    /// [`Opener::num_splits`].
    fn open_split(&mut self, index: usize) -> io::Result<File>;

    /// Total number of split zip files.
    fn num_splits(&self) -> usize;
}

/// Present a set of split files as a single joined read-only view.
///
/// The file size of each split is queried once in [`Self::new`]. Files are not
/// opened for reading until they are needed in [`Self::read`]. If EOF occurs in
/// a split before the overall EOF is reached, [`Self::read`] will return an
/// error.
///
/// Note that a single [`Self::read()`] call will correspond to a single read
/// system call and thus, will not cross split boundaries. This way, retryable
/// failures in the underlying [`File`] remain retryable through this joined
/// view.
pub struct JoinedFile<O: Opener> {
    opener: O,
...
impl<O: Opener> JoinedFile<O> {
    /// Create a new joined view instance. This will sequentially open each
    /// split to compute the boundaries and total size. The splits are not
    /// reopened again until they are needed in [`Self::read`].
    pub fn new(mut opener: O) -> io::Result<Self> {
        let mut splits = Vec::<Range<u64>>::new();

        for i in 0..opener.num_splits() {
            let file = opener.open_split(i)?;
            let size = file.metadata()?.len();
            let start = splits.last().map_or(0, |s| s.end);
            let end = start.checked_add(size).ok_or_else(|| {
                io::Error::new(
                    io::ErrorKind::InvalidData,
                    format!("File #{i} overflows joined file size"),
                )
            })?;
        ...
        let cur_split = if !splits.is_empty() { Some(0) } else { None };
    ...
    fn ensure_opened(&mut self) -> io::Result<()> {
        ...
        let file = self.opener.open_split(cur_split)?;

        self.cur_file = Some((cur_split, file));

        Ok(())
    }
}

impl<O: Opener> Read for JoinedFile<O> {
...
impl<O: Opener> Seek for JoinedFile<O> {
...
impl<R: Read + Seek> MemoryCowFile<R> {
    /// Create a new instance. This will fail if the size of the reader cannot
    /// be computed.
